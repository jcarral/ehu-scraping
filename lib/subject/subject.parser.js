...
const _parseSubjectSummary = (data, school, degree) => new Promise((resolve, reject) => {
	...
	subject.href = getSubjectUrl(subject.code, subject.school.code, subject.degree.code, subject.course);
...
const getScheduleFromTable = ($, node, title) => {
	let listOfWeeks = [];
	$(node).find('tbody>tr').each(function(i){
		const weeks = {};
...
const getTeacher = ($, node) => {
	...
	teacher['code_degree'] = getUrlCode('p_cod_plan', currentTeacherUrl);
