...
const SUMMARY_CATEGORY = 0;
const SUMMARY_DEPARTAMENT = 1;
const SUMMARY_AREA = 2;
const SUMMARY_MAIL = 3;
...
const getWeekInfo = ($, cell) => {
  ...
  weekInfo.dayStart = parseInt(rawText[4].trim(), 10);
  weekInfo.dayEnd = parseInt(rawText[6].trim(), 10);
  return weekInfo;
};
...
const getTutorship = ($, cell, weekInfo, index) => {
  ...
  if (weekInfo.dayStart > weekInfo.dayEnd) {
    const monthDays = getMonthDays(parseInt(weekInfo.year, 10), weekInfo.month);
...
const _parseTeacherSchedule = (data, degree, id) => new Promise((resolve, reject) => {
  ...
  teacher.href = EHUrls.getTeacherProfile(degreeName, id);
