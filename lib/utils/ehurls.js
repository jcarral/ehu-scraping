 ...
 */
  static getDegree(degree, school) {
    if (!degree || !school) throw new Error('Error: Degree and school codes are required. [EHUrls.getDegree]');
 ...
 */
  static getSubjectsPage(degree, school) {
    if (!degree || !school) throw new Error('Error: Degree and school codes are required. [EHUrls.getSubjectsPage]');  
 ...
 */
  static getSubject(subject, school, degree, course) {
    ...
    return `http://gestion-servicios.ehu.es/pls/entrada/plew0040.htm_asignatura_next?p_sesion=&p_cod_idioma=CAS&p_en_portal=N&p_cod_centro=${school}&p_cod_plan=${degree}&p_anyoAcad=act&p_pestanya=3&p_menu=principal&p_cod_asig=${subject}&p_ciclo=X&p_curso=${course}&p_vengo_de=asig_cursos`;
  }

  /**
 * Returns the teacher list url
 * @param {string} degreeName Degree full name
 */
  static getTeacherList(degreeName) {
    return `https://www.ehu.eus/es/web/estudiosdegrado-gradukoikasketak/${degreeName}-profesorado`;
  }
